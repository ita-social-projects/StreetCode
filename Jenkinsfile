        ...
        stage('Build') {
            steps {
                script {
                    ...
                    env.GITVERSION_PROPERTIES = sh(script: 'dotnet gitversion', returnStdout: true)
                    echo "${env.GITVERSION_PROPERTIES}"
                    env.CODE_VERSION = sh(script: "echo ${env.GITVERSION_PROPERTIES} | grep -oP '(?<=\"MajorMinorPatch\": \")[^\"]*'", returnStdout: true)
