...
pipeline {
    ...
    stages {
        ...
        stage('Setup dependencies') {
            steps {
                sh 'set +e'
                sh(script: 'dotnet tool install --global dotnet-coverage', returnStdout: false) || true
                sh(script: 'dotnet tool install --global dotnet-sonarscanner', returnStdout: false) || true
                sh(script: 'dotnet tool install --global GitVersion.Tool --version 5.12.0', returnStdout: false) || true
                sh 'set -e'
